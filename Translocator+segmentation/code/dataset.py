...
parser.add_argument('--seg_channels', default= 150, help="Number of Segmentation Channels", type=int)
parser.add_argument('--scene_of_interest', default= 2, help="indoor - 0, natural - 1, urban - 2, all - 3", type=int)
parser.add_argument('--cuda_base', default=0, help="in form cuda:x")
parser.add_argument('--device_ids', help='delimited list input',
                    type=lambda s: [int(item) for item in s.split(',')])
parser.add_argument('--no_of_scenes', default= '365', help="specify the number of scenes used (3/16/365")
parser.add_argument('--epochs', default= 15, help="number of epochs", type=int)
parser.add_argument('--dataloader_length', default= 30000, help="number of dataloader", type=int)
...
class MsgPackIterableDatasetMultiTargetWithDynLabels(torch.utils.data.IterableDataset):
    ...
    def _process_sample(self, x):
        ...
        #    img = torchvision.transforms.Resize((320,320))(img) # size = (320, 320)
        ...
        if self.meta_path is None:
            ...
            slash_id = _id.replace('/', '_') # 43_64_23423412.png
            
            img_file = self.seg_path + slash_id # ~/43_64_23423412.jpg

            seg_file = os.path.join(img_file.replace('.jpg', '.png')) # 43/64/23423412.png


        if os.path.exists(seg_file):
            #print('seg_file exists')
            ...
            if self.meta_path is None:
                ...
                _id = x[self.key_img_id].decode("utf-8") # image name
                meta = self.meta.loc[_id]
                slash_id = _id.replace('/', '_') # 43_64_23423412.jpg
                ...
                with Image.open(seg_file) as img_seg:
                    ...
                    if img_seg.width > 320 and img_seg.height > 320:
                        img_seg = torchvision.transforms.Resize((320,320))(img_seg)   
...
#                        csv_file = os.path.join(img_file.replace('.jpg', '.csv'))
                        csv_file = self.seg_path + slash_id
                        #print("csv_file name:",csv_file)

                        csv_file = csv_file.replace('.jpg', '.csv')
...
#                         print('class_idx : ', class_idx)
                        ...
                        a = seg_channels
                        #print("seg_channels:",seg_channels)
                        for c in range(class_idx.shape[0]):
                            RGB_idx = color_categories[class_idx[c]-1,:]
#                             print(np.array(img_seg).shape) # 10, 3, 224, 224
#                             print((RGB_idx).shape) # 3
                            mask = (np.array(img_seg).transpose(0,2,3,1)  == RGB_idx).all(-1)
                            # print('%')
                ...
                return img, temp, x["target"], meta[self.lat_key], meta[self.lon_key], meta[self.scene_key_2]
    ...
    def __iter__(self):
        ...
        cache = []
        #print("shard_indices",shard_indices_split)
        ...
        for shard_index in shard_indices_split:
            ...
            with open(
                ...
                #print("shard opened--",os.path.join(shard["path"], f"shard_{shard['shard_index']}.msg"))
                ...
                for x in unpacker:
                    ...
                    try:
                        ...
                        if len(self.target_mapping[_id]) == 1:
                            #print("self.targetmapping id ==1 인 경우")
...
class MultiPartitioningClassifier():
    ...
    def train_dataloader(self):
        ...
        dataset = MsgPackIterableDatasetMultiTargetWithDynLabels(
            ...
            target_mapping=target_mapping, # Dict{str, int}
            key_img_id=self.hparams.key_img_id, # image id name for msgpack dataset
            key_img_encoded=self.hparams.key_img_encoded, # image data name for msgpack dataset
            shuffle=True,
            transformation=tfm,
            transformation_seg = tfm_seg,
            meta_path=self.hparams.train_meta_path, # /resources/mp16_places365.csv
            scene_of_interest=scene_of_interest, #???????????? scene_of_interest 어디서 받음?
            seg_channels=seg_channels, # ???????????? seg_channels 어디서 받음?
            percent_seg_pixels=self.hparams.percent_seg_pixels, # 1
            cache_size=1024,
            dataloader_length=29999
        )
    ...
    def val_dataloader(self):
        ...
        with open(self.hparams.val_label_mapping, "r") as f:
            ...
            # filter the target_mapping by images with specific scene kind 
            if scene_of_interest == 0 or scene_of_interest == 1 or scene_of_interest == 2: #???????????? scene_of_interest 어디서 받음?
